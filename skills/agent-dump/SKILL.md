...
1. 识别任务模式
- 用户给了 `opencode://...`、`codex://...`、`kimi://...`、`claude://...`、`cursor://...` 这类 URI：使用 URI 模式。
...
- 保留用户显式给出的 `--collect-mode pm|insight`、`--dry-run`、`--save`、`--search`、`--reindex`、`--no-metadata-summary` 参数。
...
- `--collect` 可接受 `agents://...` 查询 URI，可使用 `--collect-mode pm|insight`、`--dry-run`、`--save`、`-since/-until`。
...
- 仅使用当前 CLI 已支持的 URI 协议：`opencode`、`codex`、`kimi`、`claude`、`cursor`、`pi`（其中 `claude` 对应 Claude Code）。
