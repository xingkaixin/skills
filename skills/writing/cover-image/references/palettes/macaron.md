# macaron

Warm educational macarons, hand-drawn classroom

## Color Palette

| Role | Color | Hex |
|------|-------|-----|
| Primary 1 | Light Blue | #A8D8EA |
| Primary 2 | Mint Green | #B5E5CF |
| Primary 3 | Lavender | #D5C6E0 |
| Background | Warm Cream | #F5F0E8 |
| Background Alt | Peach | #F4C7AB |
| Accent 1 | Coral Red | #E8655A |
| Accent 2 | Warm Gray | #6B6B6B |

## Decorative Hints

- Rounded cards, bubbles, dashed boxes as containers
- Doodle decorations: stars, underlines, small arrows
- Color blocks don't completely fill outlines
- Hand-drawn wavy arrows connecting elements
- Generous whitespace, paper grain texture

## Semantic Constraint

Do NOT render color names, hex codes, or role labels as visible text in the image.

## Best For

Educational infographics, tutorials, slide summaries, concept explanations, teaching materials, study notes
